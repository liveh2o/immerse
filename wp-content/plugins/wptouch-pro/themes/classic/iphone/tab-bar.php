	...
	<div id="menu-container">
		...
		<div id="menu-tab5">
			...
			<?php } else { ?>
				...
				<?php if ( classic_show_account_tab() ) { ?>
					<p><?php echo sprintf( __( "Not registered yet?<br />You can %ssign-up here%s.", "wptouch-pro" ), '<a class="no-ajax" href="' . wptouch_get_bloginfo( 'wpurl' ) . '/wp-register.php">','</a>' ); ?>
					</p>
					<p><?php echo sprintf(__( "Lost your password?<br />You can %sreset it here%s.", "wptouch-pro" ), '<a class="no-ajax" href="' . wptouch_get_bloginfo( 'wpurl' ) . '/wp-login.php?action=lostpassword">','</a>' ); ?>
					</p>					
				<?php } ?>
			</form>
			<?php } ?>
