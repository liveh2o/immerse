<?php 
	...
	if ( wptouch_theme_supports_ipad() ) {
		if ( $settings->ipad_support === 'full' ) {  
			$ipad_support = '(<a href="#pane-3" class="wptouch-admin-switch" rel="ipad-settings">' . __( 'iPad theme ON', "wptouch-pro" ) . '<a/>)'; 
		} else { 
			$ipad_support = '(<a href="#pane-3" class="wptouch-admin-switch" rel="ipad-settings">' . __( 'iPad Theme OFF', "wptouch-pro" ) . '<a/>)';
		}
	}	
?>
<ul>
	...
	<?php if ( wptouch_has_proper_auth() && !$settings->admin_client_mode_hide_licenses ) { ?>	
		...
		<?php } else { ?>
			<?php _e( "Status", "wptouch-pro" ); ?>: <span class="status-unl"><?php _e( 'ACTIVATION REQUIRED', 'wptouch-pro' ); ?></span> | <a href="#pane-5" id="status-target-pane-5" class="wptouch-admin-switch" rel="account" class="blue-text"><?php _e( 'Activate a Site License', 'wptouch-pro' ); ?></a>
		<?php } ?>
