			...
			onComplete: function( file, response ) {
				if ( response == 'invalid' ) {
					...
					setTimeout( function() {
							...
							jQuery( '#manage-icon-set-area li a:last' ).click();
					}, 250 );					
...
/* Remove Client Mode options for non-developers */
	...
	jQuery( '.theme-wrap' ).mouseenter( function() {
		jQuery( 'ul.option-list, p.location', this ).fadeIn();
	});

	jQuery( '.theme-wrap' ).mouseleave( function() {
		jQuery( 'ul.option-list, p.location', this ).fadeOut();
	});
...
function WPtouchSetupSelects() {
	...
	jQuery( '#advertising_type' ).change( function() {
		...
		switch( jQuery( this ).val() ) {
			case 'none':
				...
				break;
			case 'google':
				jQuery( googleDiv + ',' + displayDiv ).fadeIn();
				jQuery( admobDiv + ',' + custDiv ).hide();
				break;
			case 'admob':
				jQuery( admobDiv + ',' + displayDiv ).fadeIn();
				jQuery( googleDiv + ',' + custDiv ).hide();
				break;
			case 'custom':
				jQuery( googleDiv + ',' + admobDiv  ).hide();
				jQuery( custDiv + ',' + displayDiv ).fadeIn();
				break;
		}
	});
	...
	jQuery( '#classic_icon_type' ).change( function() {
		...
		switch( jQuery( this ).val() ) {
			case 'calendar':
				jQuery( calDiv ).fadeIn();
				jQuery( ThumbDiv + ', ' + customThumbDiv ).hide();
				break;	
			case 'thumbnails':
				jQuery( ThumbDiv ).fadeIn();
				jQuery( calDiv + ', ' + customThumbDiv ).hide();
				break;	
			case 'custom_thumbs':
				jQuery( ThumbDiv + ', ' + customThumbDiv ).fadeIn();
				jQuery( calDiv ).hide();
				break;	
			case 'none':
	...
	jQuery( '#classic_ipad_content_bg' ).change( function() {
		switch( jQuery( this ).val() ) {
			case 'custom':
				jQuery( '#setting_classic_ipad_content_bg_custom, #setting_classic_ipad_background_repeat' ).fadeIn();
				break;	
			default:
	...
	jQuery( '#classic_ipad_sidebar_bg' ).change( function() {
		switch( jQuery( this ).val() ) {
			case 'custom':
				jQuery( '#setting_classic_ipad_sidebar_bg_custom' ).fadeIn();
				break;	
			default:
	...
	var selectElements = 
		'#developer_mode,' +
		'#custom_menu_name,' +
		'#home_page_redirect_target,' +
		'#advertising_type,' +
		'#classic_calendar_icon_bg,' +
		'#classic_icon_type,' +
		'#ipad_support,' +
		'#classic_ipad_content_bg,' +
		'#classic_ipad_sidebar_bg,' +
		'.section-body-style-settings input.text,' +
		'#setting_ipad-style-settings input.text,' +
		'#backup_or_restore';

	 	jQuery( selectElements ).change();
}
...
function WPtouchSetupLicenseArea() {		
	jQuery( 'a.wptouch-remove-license' ).live( 'click', function( e ) {
		...
		jQuery( '#wptouch-license-area' ).animate( { opacity: 0.4 } ).append( '<div id="loading-big"></div>' );		
		var ajaxParams = {
			site: siteToRemove	
		};
	...
	jQuery( 'a.wptouch-add-license' ).live( 'click', function( e ) {
		jQuery( this ).unbind();
	...
	jQuery( '#reset-licenses' ).live( 'click', function( e ) {
